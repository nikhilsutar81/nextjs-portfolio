                ...
                <div className="flex items-center justify-center flex-wrap gap-4 font-lato">
                    {
                        socials?.map((social: SocialType, index: number) => {
                            ...
                            return (
                                <ScrollFadeIn key={social?.id} direction="up" delay={index * 0.1} distance={20}>
                                    <ActionTooltip side='top' label={social?.label}>
                                        <Link href={social?.link} target="_blank" className="group flex items-center hover:scale-105 transition-all duration-100">
                                            <Icon
